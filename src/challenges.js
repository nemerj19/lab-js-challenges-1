...
function howManyTimes(wordsArr, searchedWord) {
  let countWord = 0;
  if (!wordsArr.length) {
    return 0;
  }
  for (let i = 0; i <= wordsArr.length; i++) {
    if (wordsArr[i] === searchedWord) {
      countWord++;
    }
  }
  return countWord;
...
function multiplyBy(numberArr, multipyNumber) {
  if (!numberArr.length) return [];
  let newArr = [];
  numberArr.forEach(function (element) {
    newArr.push(element * multipyNumber);
  });
  // numberArr.forEach((e) => newArr.push(e * multipyNumber));
  //Es6
  return newArr;
}
//*foreach doesnt mutate the array//

// Iteration 4 | Filter Out
const original = ["cat", "dog", "fish", "bird", "cat", "fish", "cat"];
const toRemove = ["cat", "dog"];

// function filterOut(original, toRemove) {
//   if (!original.length) return null;
//   if (!toRemove.length) return original;
